...
from stable_baselines3.ppo.policies import CnnPolicy as ppoCnn



import custom_obs_policy
from base_callbacks3 import EvalCallback, TrainingTimeCallback, SaveVecNormalizeCallback
    ...
    def _on_step(self) -> bool:
        ...
        if len(history) is not 0 and self.num_timesteps is not self.old_timestep:
            if self.num_timesteps % self.log_freq == 0:
                logging.info("model: {} Success Rate: {} Timestep Num: {} lambda: {}".format(self.model_name, sr,
                                                                                             self.num_timesteps,
                                                                                             curr._lambda))
            if self.tf:
...
class SBPolicy:
    ...
    def learn(self):
        ...
        tensorboard_file = None if self.config[self.algo][
                                       'tensorboard_logs'] is None else "tensorboard_logs/" + self.model_dir
        if self.algo == 'SAC':
            if not self.env.envs[0].is_simplified() and (self.env.envs[0].depth_obs or self.env.envs[0].full_obs):
                policy_kwargs = {
                    "net_arch" : [dict(pi=self.config[self.algo]['layers'], vf=self.config[self.algo]['layers'])]
                }
                # policy_kwargs = {
                #     "net_arch": self.config[self.algo]['layers'],
                #     "features_extractor_class": custom_obs_policy.create_augmented_nature_cnn(1)}
                policy_kwargs = {}
                policy = sacCnn
        ...
        try:
            model.learn(total_timesteps=int(self.config[self.algo]['total_timesteps']),
                        callback=[
                            TensorboardCallback(self.env, tensorboard_file, self.algo, self.log_freq, self.model_dir),
                            eval_callback])
