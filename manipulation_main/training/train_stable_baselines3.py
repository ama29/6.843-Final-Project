...
def train(args):
    ...
    if args.timefeature:
        test_env = DummyVecEnv(
            [lambda: TimeFeatureWrapper(gym.make('gripper-env-v0', config=config_eval, evaluate=True, validate=True))])
    else:
        test_env = DummyVecEnv([lambda: gym.make('gripper-env-v0', config=config_eval, evaluate=True, validate=True)])

    sb_help = sb_helper3.SBPolicy(env, test_env, config, model_dir,
                                 args.load_dir, algo)
    sb_help.learn()
    env.close()
    test_env.close()
